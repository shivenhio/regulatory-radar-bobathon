...
# ---------------------------------------------------------------------------
# Deadline → priority helper
# ---------------------------------------------------------------------------

def _assign_priority(deadline_str: str) -> str:
    """
    Return 'high', 'medium', or 'low' based on how far away the deadline is.

    high   — overdue or due within 30 days
    medium — due within 31–90 days
    low    — due after 90 days (or unparseable deadline)
    """
    try:
...
# Priority sort key: high first, then medium, then low, then soonest deadline.
...
def find_gaps_for_all_rules(
    ...
    # ------------------------------------------------------------------
    # DEDUPLICATE — drop any repeated (rule_id, partner_id, product_id)
    # The five-filter scan should never produce duplicates, but if two
    # catalog rules ever share an id this acts as a safety net.
    # ------------------------------------------------------------------
    seen: set = set()
    deduped: list = []
    for f in all_findings:
        key = (f["rule_id"], f["partner_id"], f["product_id"])
        if key in seen:
            print(
                f"         DEDUP: dropped duplicate "
                f"{f['rule_id']} / {f['partner_id']} / {f['product_id']}"
            )
        else:
            seen.add(key)
            deduped.append(f)

    dropped = len(all_findings) - len(deduped)
    if dropped:
        print(f"         Deduplication    : {dropped} duplicate(s) removed")

    # ------------------------------------------------------------------
    # PRIORITIZE — add priority field and sort by urgency then deadline
    # ------------------------------------------------------------------
    for f in deduped:
        f["priority"] = _assign_priority(f["deadline"])
    ...
    # ------------------------------------------------------------------
    # AUDIT LOG — append one JSONL line per finding (bob_rules.md)
    # Records: rule → source → company → action, with timestamp.
    # File grows across runs so history is preserved.
    # ------------------------------------------------------------------
    ...
    print(f"         Audit log        : {len(deduped)} entries appended → {audit_path.name}")
