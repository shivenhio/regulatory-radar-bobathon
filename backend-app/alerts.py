...
def send_alerts_for_findings(findings_file: str = DEFAULT_FINDINGS_FILE) -> int:
    ...
    for finding in findings:
        ...
        try:
            ...
            print(f"    WARNING: Twilio error — {exc}")

        # Append one audit entry per alert attempt.
        with audit_path.open("a", encoding="utf-8") as af:
            af.write(json.dumps({
                ...
                "channel":      finding.get("alert", {}).get("channel", ""),
            }, ensure_ascii=False) + "\n")
    ...
    print(f"         Sent to         : {to}")
    print(f"         Audit log       : {sent} attempt(s) appended → {audit_path.name}")
